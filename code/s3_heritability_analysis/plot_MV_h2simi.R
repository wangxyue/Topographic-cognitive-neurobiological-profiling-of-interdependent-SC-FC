...
setwd("G:/interdependent_SCFCnet/multi_modanalysis/Rplotres/plot_MV_h2")
df<-read.csv("MV_similarity.csv")
head(df)
 
df$variable <- factor(df$variable,levels = c('MZ','DZ','SIB'))
...
p + geom_flat_violin(aes(fill=variable),
  ...
  scale_fill_manual(values=c("MZ"="#F0AD80","DZ"="#9EBBB1","SIB"="#73A0CB"))+
  scale_color_manual(values=c("MZ"="#F0AD80","DZ"="#9EBBB1","SIB"="#73A0CB"))+
  scale_y_continuous(limits=c(-0.6,1),breaks=seq(-0.6,1,0.3))+
  labs(x=" ",y="Similarity of modular variability")+
  ggtitle(" ")+
  theme_bw()+
  ...
  theme(panel.border = element_rect(colour="white",size=.3))+
  geom_signif(comparisons = list(c("MZ", "DZ"),c("MZ","SIB")),y_position = c(0.89,0.74), 
              map_signif_level = T, textsize = 14,color = "black",tip_length = c(0, 0, 0 ,0))+
  annotate('text',x=3.1,y=1.1,label='*** p < 0.001',size=6,color="black",fontface="bold")

ggsave("G:/interdependent_SCFCnet/multi_modanalysis/analysis_resfigs/MVh2sim.tiff",width=20,height=16,dpi=300)
