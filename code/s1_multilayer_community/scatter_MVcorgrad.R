...
setwd("G:/interdependent_SCFCnet/multi_modanalysis/Rplotres/plot_scatter/MV_corr_gradient")

data<-read.csv("data.csv",row.names = 1,header = T,sep = ",",stringsAsFactors = F)

MV_column<-'MV'
grad_column<-'Cortical.gradient'

X_column<-data[,MV_column]
Y_column<-data[,grad_column]

cor_R<-cor(x=X_column,y=Y_column,method='pearson')
cor_p<-cor.test(x=X_column,y=Y_column)$p.value
result<-data.frame(source_variable="MV",target_variable="Cortical.gradient",
                   cor_R=cor_R,cor_p=cor_p)

plot_example<-ggplot(data,aes_string(x="MV",y="Cortical.gradient"))+
  geom_point(size=10,color='#2570A4',alpha=3)+
  labs(x = "Multilayer modular variability", y = "Cortical gradient")+
...
ggsave("G:/interdependent_SCFCnet/multi_modanalysis/analysis_resfigs/MVcorgrad.tiff",width=20,height=16,dpi=300)